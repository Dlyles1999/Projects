 ...
 * Date: 4/15/19
 * Purpose: an application that enables you to randomly record heart rates at four different times throughout the day 
 */
using System;
...
namespace HeartBeatMonitor
{
    class HeartBeatMonitorApp
    {
        static void Main()
        {
            ...
            WriteLine("\n");
            WriteLine("\n\n");
            PrintData(heartBeat, name);
            StreamWriter SW = new StreamWriter(@"data.txt");
            int sum = 0;
            double average = 0;
            for (int i = 0; i < heartBeat.GetLength(0); i++)
            {
                for (int j = 0; j < heartBeat.GetLength(1); j++)
                {
                    sum += heartBeat[i, j];
                }
                average = sum / 4;
        ...
        public static void PrintData(int [,] hBeat, ArrayList name )
        {
            int sum = 0;
            double average = 0;
